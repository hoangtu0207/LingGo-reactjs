...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <div
        className={`bg-blue-300 text-black transition-all flex flex-col ${sidebarOpen ? "w-64" : "w-20"
          }`}
      >
        ...
        <nav className="mt-8 flex-1">
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center px-4 py-3 hover:bg-blue-400 transition ${isActive(item.path) ? "bg-blue-600 border-l-4 border-blue-500" : ""
                }`}
            >
